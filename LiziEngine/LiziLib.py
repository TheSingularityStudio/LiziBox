...
def sum_adjacent_vectors(grid, x, y, include_self=True):
    """
    ...
    for dx, dy in neighbors:
        ...
        if 0 <= nx < w and 0 <= ny < h:
            if isinstance(grid, np.ndarray):
                ...
                vx, vy = grid[ny][nx]
            sum_x += vx * 1  # 对邻居值也应用衰减
            sum_y += vy * 1
            count += 1

    return (sum_x, sum_y)

def update_grid_with_adjacent_sum(grid, include_self=False):
    """
...
void main() {
    ...
    vec2 sum = vec2(0.0);
    if(u_include_self != 0) sum += imageLoad(img_in, coord).xy * 0.2;  // 对自身值应用更小的权重
    ivec2 n;
    n = coord + ivec2(0, -1); if(n.y >= 0) sum += imageLoad(img_in, n).xy * 0.2;  // 对邻居值应用衰减
    n = coord + ivec2(0,  1); if(n.y < dims.y) sum += imageLoad(img_in, n).xy * 0.2;
    n = coord + ivec2(-1, 0); if(n.x >= 0) sum += imageLoad(img_in, n).xy * 0.2;
    n = coord + ivec2( 1, 0); if(n.x < dims.x) sum += imageLoad(img_in, n).xy * 0.2;
...
def gpu_init_compute(width=640, height=480, init_grid=None):
    ...
    """
    prog = _compile_compute_shader(COMPUTE_SHADER_SRC)
...
def gpu_step(ctx, include_self=False):
    ...
    groups_x = (ctx["w"] + 15) // 16
    groups_y = (ctx["h"] + 15) // 16
